...
- `python tray.py`：**推荐入口（Windows）**——启动后以系统托盘图标常驻后台，自动管理监控进程；右键菜单可显示仪表盘 / 重新登录 / 退出。
- `python app.py`：**常驻入口**，启动仪表盘 + 后台监控（无托盘，适合服务器/开发）。
- `python run.py`：**运维入口**，仅用于 `run.py --login` 扫码登录、`--once` 单轮检查、`--stats`/`--check` 状态检查。

三者共用同一套 SQLite 数据与浏览器登录态（`data/`、`browser_profile/`），**不要同时常驻多个入口**（`app.py` 已有单实例保护）。
